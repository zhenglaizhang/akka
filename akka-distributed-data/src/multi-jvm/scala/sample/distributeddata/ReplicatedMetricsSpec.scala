...
class ReplicatedMetricsSpec extends MultiNodeSpec(ReplicatedMetricsSpec) with STMultiNodeSpec with ImplicitSender {
  ...
  "Demo of a replicated metrics" must {
    ...
    "replicate metrics" in within(30.seconds) {
      ...
      awaitAssert {
        val size = probe.expectMsgType[UsedHeap](1.second).percentPerNode.size
        log.info("First run: {}", size)
        size should be(3)
      }
      for (_ ← 1 to 20) {
        val size = probe.expectMsgType[UsedHeap].percentPerNode.size
        log.info("Second run: {}", size)
        size should be(3)
      }
